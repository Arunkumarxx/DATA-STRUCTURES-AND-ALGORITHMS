...
public class StockSpanProblem {
    ...
    private class StackNode {
        ...
        StackNode(int data1, int data2) {
            this.data = new int[2]; // Initialize the array
            this.data[0] = data1;
            this.data[1] = data2;
            this.next = null;
        }
    ...
    private int[] top() {
        return head != null ? head.data : null; // Check if head is not null
    }

    private int[] pop() {
        if (isEmpty()) return null; // Handle empty stack case
        int[] res = head.data;
        head = head.next;
        return res;
    }
